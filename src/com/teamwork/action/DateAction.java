...
//
	...
	public String findAllDate() {
		...
		if (sign==1) {
			pagePath = "./user/everyonedate.jsp";
		} else {
	...
	public String findAllDateById() {
		...
		List l = this.dateService.userDate(id);
		ActionContext.getContext().put("mydate", model);
		ActionContext.getContext().put("userDate",l.get(0));
	...
	public String findDateById() {
		...
		int id = Integer.parseInt(request.getParameter("id").toString());
		Date d = this.dateService.getDateByID(id);
		User u = this.dateService.getUserByID(d.getUserId());
		PageModel model_service = this.dateService.findAllService();
		ActionContext.getContext().put("model_service", model_service);
		ActionContext.getContext().put("dateById", d);
		ActionContext.getContext().put("dateById_user", u.getUserName());
		if (sign==1) {
			pagePath = "./user/userdate.jsp";
		} else {
...
//		date.setUsers(ts);
	...
	public String addApply() {
		...
		User u = (User)session.getAttribute("user");
		Set<User> su = d.getUsers();
		su.add(u);
		d.setUsers(su);
		dateService.updateDate(d);
		pagePath = "./user/everyonedate.jsp";
		return "pagePath";
	}
